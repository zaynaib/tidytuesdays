...
ukraine_finacial_aid_alpha$alphayear = "a"

ukraine_finacial_aid_alpha[33,1:2]
ukraine_finacial_aid_alpha[33,3] <- "b"


ukraine_finacial_aid_alpha %>%
  filter(`Fiscal Year` >= 2000  & `Fiscal Year` < 2023) %>%
  ggplot(aes(x=`Fiscal Year`,y=sum_aid_dollars))+
  geom_bar(stat='identity',fill = "#C88967",alpha=factor(alphayear)) 
